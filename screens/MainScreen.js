...
function NavItem({ name, isActive, onPress }) {
  const IconComponent = name === 'map-marker' ? MaterialCommunityIcons : Ionicons;
  const size = name === 'map-marker' ? 40 : 30;

  return (
    <TouchableOpacity onPress={onPress} style={styles.navItem}>
      <IconComponent name={name} size={size} color={isActive ? 'rgb(2, 150, 255)' : '#000'} />
    </TouchableOpacity>
  );
}
...
export default function MainScreen() {
  ...
  const myUser = getObjectById(getUserId(), people);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.navBar}>
        ...
        <NavItem name="person" isActive={activeTab === 'myProfile'} onPress={() => setActiveTab('myProfile')} />
      </View>
    </View>
  );
}
